var dendrogram = {
	...
	draw : function(l) {
		...
		//rotate the canvas if necessary
			if(dendrogram.orientation=='left'){
				...
				dendrogram.floor = dendrogram.canvas.width-dendrogram.leftOffset;
				dendrogram.ceil = dendrogram.rightOffset;
				dendrogram.wallR = dendrogram.canvas.height-dendrogram.bottomOffset;
	...
	canvasConfigure : function (ctx) {
			if(dendrogram.border){
				//ctx.beginPath();
				ctx.moveTo(dendrogram.wallL, dendrogram.floor);
				ctx.lineTo(dendrogram.wallL, dendrogram.ceil);
				ctx.lineTo(dendrogram.wallR, dendrogram.ceil);
				ctx.lineTo(dendrogram.wallR, dendrogram.floor);
				ctx.lineTo(dendrogram.wallL, dendrogram.floor)
				ctx.stroke();
			}
	...
	drawCluster : function (cl, cluster) {
		...
		//height will need to be subtracted from max deepness
			...
			dendrogram.ctx.beginPath();
			dendrogram.ctx.strokeStyle = "rgb(0,0,255)";
			dendrogram.ctx.fillStyle = "rgb(0,0,255)";
			...
			//now create the labels below the x axis; only the labels in input groups should be new; other groups are clusters
			var groupA = cl[0];var groupB = cl[1];
			
			
			if(typeof(dendrogram.clusterMidpointX[groupA])==='undefined'){//where the cluster is NOT defined yet, add a label
				if(typeof(dendrogram.labels[groupA])==='undefined'){
					dendrogram.labels[groupA] = groupA;
				}
				var labelA = dendrogram.labels[groupA];
				
				if(dendrogram.xticks){
					dendrogram.ctx.fillText(labelA, point3[0]-dendrogram.ctx.measureText(labelA).width/2, dendrogram.maxDeepeness+15);
				}
				
			}
			
			if(typeof(dendrogram.clusterMidpointX[groupB])==='undefined'){
				if(typeof(dendrogram.labels[groupB])==='undefined'){
					dendrogram.labels[groupB] = groupB;
				}
				var labelB = dendrogram.labels[groupB];
				
				if(dendrogram.xticks){
					dendrogram.ctx.fillText(labelB, point1[0]-dendrogram.ctx.measureText(labelB).width/2, dendrogram.maxDeepeness+15);
				}
				
				
				
			}
			if(typeof(labelB)!=='undefined'){
				dendrogram.groupLabel.push(labelB);
				dendrogram.newGroups.push(groupB);
			}
			if(typeof(labelA)!=='undefined'){
				dendrogram.groupLabel.push(labelA);
				dendrogram.newGroups.push(groupA);
			}

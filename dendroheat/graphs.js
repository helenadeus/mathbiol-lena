var graphs  = {// this variable will have elements, and methods, to manipulate the graphical object
	...
	configureDendrogram : function (canvasid) {
		...
		dendrogram.holder = 'right';
		dendrogram.width = 400;
		dendrogram.height = $('#'+data.tableid).height();
		dendrogram.style = '';
		dendrogram.LineWidth = 3;
		dendrogram.orientation = 'left';
		dendrogram.topOffset = 1;
		dendrogram.bottomOffset = 0;
		dendrogram.leftOffset = 1;
		dendrogram.rightOffset = 1;
		dendrogram.border = 0;
		dendrogram.xticks = 0;
		dendrogram.yticks = 0;
		dendrogram.grid = 0;
		   //dendrogram.canvas = document.getElementById(canvasid);
	...
	makeDendrogram : function (rawData) {
			...
			var cW = dendrogram.height; var cH = dendrogram.height; var fL = (3*cW)/(2*n); var lL = cW-(cW/n)/2; var jp = (lL-fL)/(n-1);
			dendrogram.fL = fL;
		    dendrogram.lL = lL;
	...
	drawDataTable : function (data_id_order) {
		if(typeof(data_id_order)=='undefined'){
			data_id_order = [];
			for (var id in data.matrix) {
				if(typeof(id)!=='undefined'){
					data_id_order.push(escape(id));
				}
			}
		}
		
		for (var i=0; i<data_id_order.length; i++) {
			var id = data_id_order[i];
			if(typeof(data.matrix[id])!=='undefined'){
				$(document.createElement('tr')).attr('height',data.rowHeight).attr('id', id).appendTo('#'+graphs.tableid);
				$(document.createElement('td')).attr('id', id+'_id').appendTo('#'+id);
				//create the checkbox
					$(document.createElement('input')).attr('type','checkbox').attr('id', 'check_'+id+'_'+varName).attr('control_inst',id).attr('checked', true).appendTo('#'+id+'_id').click(function (e) {
						console.log($(this).attr('control_inst')+' was '+(($(this).attr('checked'))?'checked':'unchecked'));				
						var varName = $(this).attr('control_inst');
						var checked = $(this).attr('checked');
						if(checked){
				...
				//create a span with this isolate's name
				var label = data.labels[id];
				$(document.createElement('span')).html(label).appendTo('#'+id+'_id');

				for (var j=0; j<data.variableSelection.length; j++) {
					var varName = data.variableSelection[j];
					if(typeof(data.matrix[id][varName])!=='undefined'){
						var color = graphs.color2rgb([data.colorMatrix[id][varName]]);
						$(document.createElement('td')).attr('id', id+'_'+varName).attr('style', 'background-color: rgb('+color[0]+')').html(data.matrix[id][varName]).appendTo('#'+id);
					}
					else {
						 $(document.createElement('td')).attr('id', id+'_'+varName).attr('style', 'background-color: rgb(255,255,255)').html('(no data)').appendTo('#'+id);
					}
				}	
			}
		}		
